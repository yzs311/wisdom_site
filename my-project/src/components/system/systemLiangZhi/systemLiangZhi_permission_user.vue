...
export default {
    ...
    methods: {
        ...
        // 获取账号列表
        querySystemUserList() {
            this.$axios.post(`/api/system/computer/querySystemUser?id=${this.userId}&parameterId=${this.parameterId}&page=${this.page}&pageSize=${this.pageSize}`).then(
                res => {
                    // console.log(res.data)
                    let temp = []
        ...
        querySystemUser() {
            if (this.userName && this.userPhone && this.userAccount && this.userPassword && this.userState && this.entry) {
                this.$axios.post(`/api/system/computer/insertSystemUser?publicId=${this.parameterId}&userName=${this.userName}&userPhone=${this.userPhone}&userAccount=${this.userAccount}&userPassword=${this.userPassword}&userState=${this.userState}&userType=${this.userType}&entry=${this.entry}&ids=${this.ids}`).then(
                    res => {
                        if (res.data.code == 0) {
                            this.$message({
                                message: '添加成功',
        ...
        updateSystemUser() {
            if (this.userName && this.userPhone && this.userAccount && this.userPassword && this.userState && this.entry) {
                this.$axios.post(`/api/system/computer/updateSystemUser?publicId=${this.parameterId}&userName=${this.userName}&userPhone=${this.userPhone}&userAccount=${this.userAccount}&userPassword=${this.userPassword}&userState=${this.userState}&userType=${this.userType}&entry=${this.entry}&ids=${this.ids}&id=${this.id}`).then(
                    res => {
                        if (res.data.code == 0) {
                            this.$message({
                                message: '修改成功',
