...
export default {
    ...
    methods: {
        ...
        // 获取电箱设备列表
        getEquipmentList() {
            this.$axios.post(`/api/ProjectElectricityBox/list?proiectId=${this.projectId}&pageNum=${this.pageNum3}&pageSize=${this.pageSize3}`).then(
                res => {
        ...
        addSave() {
            if (this.addComments && this.addElectricityBoxId) {
                this.$axios.post(`/api/ProjectElectricityBox/addSave?projectId=${this.projectId}&comments=${this.addComments}&electricityBoxId=${this.addElectricityBoxId}`).then(
                    res => {
                        // console.log(res.data)
                        if (res.data.code == 0) {
                            this.$message({
        ...
        removeClick(ids) {
            this.$confirm('是否要删除选中的参建单位？', '提示', {
                ...
                this.$axios.post(`/api/ProjectElectricityBox/remove?ids=${ids}`).then(
                    res => {
                        // console.log(res.data)
                        if (res.data.code == 0) {
                            this.$message({
        ...
        // 获取编辑设备信息
        getEdit(id) {
            this.$axios.post(`/api/ProjectElectricityBox/edit?id=${id}`).then(
                res => {
                    // console.log(res.data)
        ...
        editClick() {
            if (this.editComments && this.editElectricityBoxId) {
                this.$axios.post(`/api/ProjectElectricityBox/editSave?projectId=${this.projectId}&comments=${this.editComments}&electricityBoxId=${this.editElectricityBoxId}&id=${this.selectId}`).then(
                    res => {
                        // console.log(res.data)
                        if (res.data.code == 0) {
                            this.$message({
